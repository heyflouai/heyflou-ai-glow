...
export const ProblemSolution = () => {
  ...
  return (
    <div className="space-y-10 md:space-y-16 px-5 md:px-0">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
        <div className="space-y-4 md:space-y-6">
          <div className="space-y-3 md:space-y-4">
            ...
            <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold font-display text-hf-ink leading-tight">{t.problemSolution.problemTitle}</h2>
          </div>
          <div className="space-y-3 md:space-y-4">
            {symptoms.map((symptom, index) => {
              const Icon = symptom.icon;
              return (<div key={index} className="flex items-start gap-3 animate-fade-in" style={{ animationDelay: `${index * 0.1}s` }}><div className="w-5 h-5 mt-0.5 text-hf-teal flex-shrink-0"><Icon size={20} /></div><p className="text-sm md:text-base text-muted-foreground">{symptom.text}</p></div>);
            })}
          </div>
        </div>
        <div className="order-first lg:order-last"><WorkflowDiagram /></div>
      </div>
      ...
      <div className="space-y-6 md:space-y-8">
        <div className="text-center max-w-3xl mx-auto space-y-3 md:space-y-4">
          ...
          <h3 className="text-xl md:text-2xl lg:text-3xl font-bold font-display text-hf-ink dark:text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 max-w-4xl mx-auto">
          {benefits.map((benefit, index) => {
            ...
            return (<div key={index} className="flex items-start gap-3 animate-fade-in group p-4 md:p-0 bg-card md:bg-transparent rounded-lg md:rounded-none border border-border/50 md:border-0" style={{ animationDelay: `${(index + 6) * 0.1}s` }}><div className="w-6 h-6 md:w-7 md:h-7 mt-0.5 flex-shrink-0 text-hf-teal dark:text-transparent dark:bg-gradient-to-br dark:from-hf-sky dark:to-hf-purple dark:bg-clip-text relative"><div className="absolute inset-0 dark:bg-gradient-to-br dark:from-hf-sky/20 dark:to-hf-purple/20 dark:blur-lg dark:rounded-full"></div><Icon size={22} className="relative dark:text-hf-sky dark:drop-shadow-[0_0_8px_hsl(var(--hf-sky)/0.5)]" /></div><p className="text-sm md:text-base text-muted-foreground dark:text-[hsl(220,25%,88%)]">{benefit.text}</p></div>);
          })}
        </div>
        <div className="text-center">
          <div className="flex flex-wrap justify-center gap-2 md:gap-3">
            {stats.map((stat, index) => (<div key={index} className="px-3 md:px-4 py-2 bg-hf-teal/10 rounded-full text-xs md:text-sm font-medium text-hf-navy animate-fade-in dark:bg-gradient-to-r dark:from-hf-purple/15 dark:to-hf-sky/15 dark:text-white dark:border dark:border-hf-purple/30 dark:hover:from-hf-purple/25 dark:hover:to-hf-sky/25 dark:hover:border-hf-sky/40 dark:hover:shadow-[0_0_20px_hsl(var(--hf-purple)/0.2)] dark:transition-all dark:duration-300" style={{ animationDelay: `${(index + 10) * 0.1}s` }}>{stat}</div>))}
          </div>
        </div>
      </div>
