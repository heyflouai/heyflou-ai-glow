      ...
      <div ref={ref} className="relative max-w-7xl mx-auto pb-20">
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className={cn("hidden md:block text-xl md:pl-20 md:text-4xl font-bold text-foreground", titleClassName)}>
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className={cn("md:hidden block text-2xl mb-4 text-left font-bold text-foreground", titleClassName)}>
                {item.title}
              </h3>
              {item.content}
            </div>
        ...
        >
          <motion.div
            ...
            className="absolute inset-x-0 top-0 w-[2px] bg-gradient-to-t from-hf-purple via-hf-teal to-transparent from-[0%] via-[10%] rounded-full"
          />
        </div>
      </div>
    </div>
